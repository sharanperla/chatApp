...
const createGroupFormSchma = z.object({
  name: z.string().min(1, { message: "This field cant be empty" }),
  members: z
    .string()
    .array()
    .min(1, { message: "you musts select atleaset one friend" }),
});
...
const CreateGroupDialog = (props: Props) => {
  ...
  const form = useForm<z.infer<typeof createGroupFormSchma>>({
    resolver: zodResolver(createGroupFormSchma),
    defaultValues: {
      name: "",
      members: [],
    },
  ...
  const handleSubmit = async (values: z.infer<typeof createGroupFormSchma>) => {
    await createGroup({
      name: values.name,
      ...
      .catch((error) => {
        toast.error(
          error instanceof ConvexError ? error.data : "Unexpected error occred"
        );
      });
  };
  return (
                    ...
                    return (
                      ...
                      >
                        ...
                        <Avatar className="w-8 h-8">
                                ...
                                <X className="text-muted-foreground w-4 h-4 absolute bottom-8 left-7 bg-muted rounded-full cursor-pointer " onClick={()=>{
